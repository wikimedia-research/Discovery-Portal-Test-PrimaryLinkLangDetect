second_visits <- readr::read_rds("data/refined_2nd_visits.rds")
...
foo <- function(data) { # data <- second_visits
...
p1 <- foo(second_visits) +
  ggtitle("Which Wikipedia the users head to from Portal on their second visit",
          subtitle = "All users")
ggsave("wikipedia_visits_all_2.png", p1, path = "figures", width = 15, height = 5, dpi = 300)
p2 <- foo(filter(second_visits, `Primary language` != "English")) +
  ggtitle("Which Wikipedia the users head to from Portal on their second visit",
          subtitle = "Excluding users whose most preferred language is English")
ggsave("wikipedia_visits_non-prime-En_2.png", p2, path = "figures", width = 15, height = 5, dpi = 300)
p3 <- foo(filter(second_visits, preferred_languages != "en")) +
  ggtitle("Which Wikipedia the users head to from Portal on their second visit",
          subtitle = "Excluding users whose only preferred language is English")
ggsave("wikipedia_visits_non-En_2.png", p3, path = "figures", width = 15, height = 5, dpi = 300)
p4 <- foo(filter(second_visits, `Number of Accept-Languages` > 1)) +
  ggtitle("Which Wikipedia the users head to from Portal on their second visit",
          subtitle = "Only multilingual users; subgroups with <10 users were excluded for quality-control")
ggsave("wikipedia_visits_multilang_2.png", p4, path = "figures", width = 15, height = 5, dpi = 300)
rm(p1, p2, p3, p4)
